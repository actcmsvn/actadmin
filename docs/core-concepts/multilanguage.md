...
Actadmin supports multiple languages for your models.To get started, you need to configure some things first.

## Setup

First you need to define some `locales` in your `config/actadmin.php` file and `enable` multilanguage:
...
```php
// with string
if (Actadmin::translatable(Post::class)) {
    // it's translatable
}

// with object of Model or Collection
if (Actadmin::translatable($post)) {
    // it's translatable
}
```
