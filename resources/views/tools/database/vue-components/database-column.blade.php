...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('Actadmin::database.index') }}</option>
            <option value="UNIQUE">{{ __('Actadmin::database.unique') }}</option>
            <option value="PRIMARY">{{ __('Actadmin::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('Actadmin::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="actadmin-trash"></i></div>
    </td>
...
@include('Actadmin::tools.database.vue-components.database-types')
@include('Actadmin::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('Actadmin::database.name_warning') }}");
                }
