...
<div class="panel panel-bordered">
    <div class="panel-body">
        ...
        @else
            <div class="col-md-6">
        @endif
                <label for="name">{{ __('Actadmin::database.table_name') }}</label><br>
                <input v-model.trim="table.name" type="text" class="form-control" placeholder="{{ __('Actadmin::database.table_name') }}" required pattern="{{ $db->identifierRegex }}">
            </div>

        @if($db->action == 'create')
            <div class="col-md-3 col-sm-4 col-xs-6">
                <label for="create_model">{{ __('Actadmin::database.create_model_table') }}</label><br>
                <input type="checkbox" name="create_model" data-toggle="toggle"
                       data-on="{{ __('Actadmin::generic.yes_please') }}" data-off="{{ __('Actadmin::generic.no_thanks') }}">
            </div>
            {{--
                Hide migration button until feature is available.
                 <div class="col-md-3 col-sm-4 col-xs-6">
                    <label for="create_migration">{{ __('Actadmin::database.create_migration') }}</label><br>
        ...
        <div v-if="compositeIndexes.length" v-once class="alert alert-danger">
            <p>{{ __('Actadmin::database.no_composites_warning') }}</p>
        </div>

        <div id="alertsContainer"></div>

        <template v-if="tableHasColumns">
            <p>{{ __('Actadmin::database.table_columns') }}</p>

            <table class="table table-bordered" style="width:100%;">
                <thead>
                <tr>
                    <th>{{ __('Actadmin::generic.name') }}</th>
                    <th>{{ __('Actadmin::generic.type') }}</th>
                    <th>{{ __('Actadmin::generic.length') }}</th>
                    <th>{{ __('Actadmin::generic.not_null') }}</th>
                    <th>{{ __('Actadmin::generic.unsigned') }}</th>
                    <th>{{ __('Actadmin::generic.auto_increment') }}</th>
                    <th>{{ __('Actadmin::generic.index') }}</th>
                    <th>{{ __('Actadmin::generic.default') }}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
        ...
        <div v-else>
          <p>{{ __('Actadmin::database.table_no_columns') }}</p>
        </div>
    ...
    <div class="panel-footer">
        <input type="submit" class="btn btn-primary pull-right"
               value="@if($db->action == 'update'){{ __('Actadmin::database.update_table') }}@else{{ __('Actadmin::database.create_new_table') }}@endif"
...
@include('Actadmin::tools.database.vue-components.database-column')
@include('Actadmin::tools.database.vue-components.database-table-helper-buttons')
        ...
        methods: {
            addColumn(column) {
                ...
                if (column.name && this.hasColumn(column.name)) {
                    return toastr.error("{{ __('Actadmin::database.column') }} " + column.name + " {{ __('Actadmin::database.already_exists') }}");
                }
            ...
            renameColumn(column) {
                ...
                if ((existingColumn = this.getColumn(newName)) && (existingColumn !== column)) {
                    return toastr.error("{{ __('Actadmin::database.column') }} " + newName + " {{ __('Actadmin::database.already_exists') }}");
                }
            ...
            addIndex(index) {
                if (index.type == 'PRIMARY') {
                    if (this.table.primaryKeyName) {
                        return toastr.error("{{ __('Actadmin::database.table_has_index') }}");
                    }
