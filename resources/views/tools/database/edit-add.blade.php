...
@if($db->action == 'update')
    @section('page_title', __('Actadmin::database.editing_table', ['table' => $db->table->name]))
@else
    @section('page_title', __('Actadmin::database.create_new_table'))
@endif
...
@section('page_header')
    <h1 class="page-title">
        ...
        @if($db->action == 'update')
            {{ __('Actadmin::database.editing_table', ['table' => $db->table->name]) }}
        @else
            {{ __('Actadmin::database.create_new_table') }}
        @endif
    </h1>
@stop
...
@section('javascript')
    @include('Actadmin::tools.database.vue-components.database-table-editor')
