...
@if (isset($dataType->id))
    @section('page_title', __('Actadmin::bread.edit_bread_for_table', ['table' => $dataType->name]))
@else
    @section('page_title', __('Actadmin::bread.create_bread_for_table', ['table' => $table]))
@endif

@section('page_header')
    <div class="page-title">
        <i class="actadmin-data"></i>
        @if (isset($dataType->id))
            {{ __('Actadmin::bread.edit_bread_for_table', ['table' => $dataType->name]) }}
        @else
            {{ __('Actadmin::bread.create_bread_for_table', ['table' => $table]) }}
        @endif
    </div>
    @php
...
<ol class="breadcrumb hidden-xs">
    <li class="active">
        <a href="{{ route('actadmin.dashboard')}}"><i class="actadmin-boat"></i> {{ __('Actadmin::generic.dashboard') }}</a>
    </li>
    <li class="active">
        <a href="{{ route('actadmin.bread.index') }}">
            {{ __('Actadmin::generic.bread') }}
        </a>
    </li>
    <li class="active">
        @if(isset($dataType->id))
        <a href="{{ route('actadmin.bread.edit', $table) }}">
            {{ $dataType->display_name_singular }}
        </a>
        @else
        <a href="{{ route('actadmin.bread.create', ['name' => $table]) }}">
            {{ $display_name }}
        </a>
        @endif
    </li>
    <li>
        {{ isset($dataType->id) ? __('Actadmin::generic.edit') : __('Actadmin::generic.add') }}
    </li>
</ol>
@endsection

@section('content')
    <div class="page-content container-fluid" id="ActadminBreadEditAdd">
        <div class="row">
            <div class="col-md-12">

                <form action="@if(isset($dataType->id)){{ route('actadmin.bread.update', $dataType->id) }}@else{{ route('actadmin.bread.store') }}@endif"
                ...
                @endif
                    ...
                    <div class="panel panel-primary panel-bordered">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="actadmin-bread"></i> {{ ucfirst($table) }} {{ __('Actadmin::bread.bread_info') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action actadmin-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="name">{{ __('Actadmin::database.table_name') }}</label>
                            ...
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="display_name_singular">{{ __('Actadmin::bread.display_name_singular') }}</label>
                                    @if($isModelTranslatable)
                                        @include('Actadmin::multilingual.input-hidden', [
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('Actadmin::bread.display_name_singular') }}"
                                           value="@if(isset($dataType->display_name_singular)){{ $dataType->display_name_singular }}@else{{ $display_name }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="display_name_plural">{{ __('Actadmin::bread.display_name_plural') }}</label>
                                    @if($isModelTranslatable)
                                        @include('Actadmin::multilingual.input-hidden', [
                                            'isModelTranslatable' => true,
                                    ...
                                    <input type="text" class="form-control"
                                           ...
                                           placeholder="{{ __('Actadmin::bread.display_name_plural') }}"
                                           value="@if(isset($dataType->display_name_plural)){{ $dataType->display_name_plural }}@else{{ $display_name_plural }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="slug">{{ __('Actadmin::bread.url_slug') }}</label>
                                    <input type="text" class="form-control" name="slug" placeholder="{{ __('Actadmin::bread.url_slug_ph') }}"
                                           value="@if(isset($dataType->slug)){{ $dataType->slug }}@else{{ $slug }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="icon">{{ __('Actadmin::bread.icon_hint') }} <a
                                                href="{{ route('actadmin.compass.index', [], false) }}#fonts"
                                                target="_blank">{{ __('Actadmin::bread.icon_hint2') }}</a></label>
                                    <input type="text" class="form-control" name="icon"
                                           placeholder="{{ __('Actadmin::bread.icon_class') }}"
                                           value="@if(isset($dataType->icon)){{ $dataType->icon }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="model_name">{{ __('Actadmin::bread.model_name') }}</label>
                                    <span class="actadmin-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('Actadmin::bread.model_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="model_name" placeholder="{{ __('Actadmin::bread.model_class') }}"
                                           value="@if(isset($dataType->model_name)){{ $dataType->model_name }}@else{{ $model_name }}@endif">
                                </div>
                                <div class="col-md-6 form-group">
                                    <label for="controller">{{ __('Actadmin::bread.controller_name') }}</label>
                                    <span class="actadmin-question"
                                        aria-hidden="true"
                                        data-toggle="tooltip"
                                        data-placement="right"
                                        title="{{ __('Actadmin::bread.controller_name_hint') }}"></span>
                                    <input type="text" class="form-control" name="controller" placeholder="{{ __('Actadmin::bread.controller_name') }}"
                                           value="@if(isset($dataType->controller)){{ $dataType->controller }}@endif">
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-6 form-group">
                                    <label for="policy_name">{{ __('Actadmin::bread.policy_name') }}</label>
                                    <span class="actadmin-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('Actadmin::bread.policy_name_ph') }}"></span>
                                    <input type="text" class="form-control" name="policy_name" placeholder="{{ __('Actadmin::bread.policy_class') }}"
                                           value="@if(isset($dataType->policy_name)){{ $dataType->policy_name }}@endif">
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="generate_permissions">{{ __('Actadmin::bread.generate_permissions') }}</label><br>
                                    ...
                                    <input type="checkbox" name="generate_permissions" class="toggleswitch" data-on="{{ __('Actadmin::generic.yes') }}" data-off="{{ __('Actadmin::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="server_side">{{ __('Actadmin::bread.server_pagination') }}</label><br>
                                    <?php $checked = (isset($dataType->server_side) && $dataType->server_side == 1) ? true : (isset($server_side) && $server_side) ? true : false; ?>
                                    <input type="checkbox" name="server_side" class="toggleswitch" data-on="{{ __('Actadmin::generic.yes') }}" data-off="{{ __('Actadmin::generic.no') }}"
                                           @if($checked) checked @endif>
                                </div>
                            </div>
                            <div class="row clearfix">
                                <div class="col-md-3 form-group">
                                    <label for="order_column">{{ __('Actadmin::bread.order_column') }}</label>
                                    <span class="actadmin-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('Actadmin::bread.order_column_ph') }}"></span>
                                    <select name="order_column" class="select2 form-control">
                                          <option value="">-- {{ __('Actadmin::generic.none') }} --</option>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_display_column">{{ __('Actadmin::bread.order_ident_column') }}</label>
                                    <span class="actadmin-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('Actadmin::bread.order_ident_column_ph') }}"></span>
                                ...
                                <div class="col-md-3 form-group">
                                    <label for="order_direction">{{ __('Actadmin::bread.order_direction') }}</label>
                                    <select name="order_direction" class="select2 form-control">
                                        <option value="asc" @if(isset($dataType) && $dataType->order_direction == 'asc') selected @endif>
                                            {{ __('Actadmin::generic.ascending') }}
                                        </option>
                                        <option value="desc" @if(isset($dataType) && $dataType->order_direction == 'desc') selected @endif>
                                            {{ __('Actadmin::generic.descending') }}
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="default_search_key">{{ __('Actadmin::bread.default_search_key') }}</label>
                                    <span class="actadmin-question"
                                          aria-hidden="true"
                                          data-toggle="tooltip"
                                          data-placement="right"
                                          title="{{ __('Actadmin::bread.default_search_key_ph') }}"></span>
                            ...
                            <div class="form-group">
                                <label for="description">{{ __('Actadmin::bread.description') }}</label>
                                <textarea class="form-control" name="description"
                                          placeholder="{{ __('Actadmin::bread.description') }}"
                    ...
                    <div class="panel panel-primary panel-bordered">
                        <div class="panel-heading">
                            <h3 class="panel-title panel-icon"><i class="actadmin-window-list"></i> {{ __('Actadmin::bread.edit_rows', ['table' => $table]) }}:</h3>
                            <div class="panel-actions">
                                <a class="panel-action actadmin-angle-up" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
                        ...
                        <div class="panel-body">
                            <div class="row fake-table-hd">
                                <div class="col-xs-2">{{ __('Actadmin::database.field') }}</div>
                                <div class="col-xs-2">{{ __('Actadmin::database.visibility') }}</div>
                                <div class="col-xs-2">{{ __('Actadmin::database.input_type') }}</div>
                                <div class="col-xs-2">{{ __('Actadmin::bread.display_name') }}</div>
                                <div class="col-xs-4">{{ __('Actadmin::database.optional_details') }}</div>
                            </div>
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                @if(isset($dataType->id))
                                    <?php $dataRow = ACT\Actadmin\Models\DataRow::where('data_type_id', '=',
                                ...
                                <div class="row row-dd">
                                    <div class="col-xs-2">
                                        ...
                                        <strong>{{ __('Actadmin::database.type') }}:</strong> <span>{{ $data['type'] }}</span><br/>
                                        <strong>{{ __('Actadmin::database.key') }}:</strong> <span>{{ $data['key'] }}</span><br/>
                                        <strong>{{ __('Actadmin::generic.required') }}:</strong>
                                        @if($data['null'] == "NO")
                                            <span>{{ __('Actadmin::generic.yes') }}</span>
                                            <input type="hidden" value="1" name="field_required_{{ $data['field'] }}"
                                                   checked="checked">
                                        @else
                                            <span>{{ __('Actadmin::generic.no') }}</span>
                                            <input type="hidden" value="0" name="field_required_{{ $data['field'] }}">
                                        @endif
                                        <div class="handler actadmin-handle"></div>
                                    ...
                                    <div class="col-xs-2">
                                        <input type="checkbox"
                                               ...
                                               @elseif(!isset($dataRow->browse))
                                                    {{ 'checked="checked"' }}
                                               @endif>
                                        <label for="field_browse_{{ $data['field'] }}">{{ __('Actadmin::generic.browse') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_read_{{ $data['field'] }}">{{ __('Actadmin::generic.read') }}</label><br/>
                                        <input type="checkbox"
                                        ...
                                        <label for="field_edit_{{ $data['field'] }}">{{ __('Actadmin::generic.edit') }}</label><br/>
                                        <input type="checkbox"
                                            ...
                                            <label for="field_add_{{ $data['field'] }}">{{ __('Actadmin::generic.add') }}</label><br/>
                                        <input type="checkbox"
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('Actadmin::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
                        ...
                        <div class="panel-footer">
                             <div class="btn btn-new-relationship"><i class="actadmin-heart"></i> <span>
                             {{ __('Actadmin::database.relationship.create') }}</span></div>
                        </div>
                    </div><!-- .panel -->

                    <button type="submit" class="btn pull-right btn-primary">{{ __('Actadmin::generic.submit') }}</button>
...
@include('Actadmin::tools.bread.relationship-new-modal')
...
@section('javascript')
    ...
    <script>
            ...
            $('textarea[data-editor]').each(function () {
                ...
                textarea.closest('form').on('submit', function (ev) {
                    if (window.invalidEditors.length) {
                        ...
                        toastr.error('{{ __('Actadmin::json.invalid_message') }}', '{{ __('Actadmin::json.validation_errors') }}', {"preventDuplicates": true, "preventOpenDuplicates": true});
                    } else {
       ...
       $(function () {
            ...
            $('.actadmin-relationship-details-btn').click(function(){
                $(this).toggleClass('open');
                if($(this).hasClass('open')){
                    $(this).parent().parent().find('.actadmin-relationship-details').slideDown();
                } else {
                    $(this).parent().parent().find('.actadmin-relationship-details').slideUp();
                }
            });
        ...
        function populateRowsFromTable(dropdown){
            ...
            if(tbl.length != 0){
                $.get('{{ route('actadmin.database.index') }}/' + tbl, function(data){
                    $(dropdown).empty();
