...
namespace ACT\Actadmin\Traits;

use Exception;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\Relations\Relation;
use ACT\Actadmin\Facades\Actadmin;
use ACT\Actadmin\Translator;
...
trait Translatable
{
    ...
    {
        return $this->hasMany(Actadmin::model('Translation'), 'foreign_key', $this->getKeyName())
            ->where('table_name', $this->getTable())
            ->whereIn('locale', config('actadmin.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('actadmin.multilingual.locales', [config('actadmin.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('actadmin.multilingual.default'), false];
        }
        ...
        $default = config('actadmin.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('actadmin.multilingual.default', 'en');
        $locales = config('actadmin.multilingual.locales', [$default]);
    ...
    public function prepareTranslations(&$request)
    {
        ...
        foreach ($transFields as $field) {
            ...
            $request->merge([$field => $trans[config('actadmin.multilingual.default', 'en')]]);
